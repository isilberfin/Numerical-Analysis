...
#include <math.h>
#define f(x) ((x*x*x)-18)
#define fd(x) (3*x*x)      //derivative of f(x)
#define fdd(x) 6*x         //second derivative of f(x)
int main(){
float x0,x1,error,error-old,converge,order;
int i=0;
...
do{
        errorold=error;
        x1=x0-(f(x0)/fd(x0));                   //formula for the next point 
        if(f(x1)==0){
                break;
        }
        error=fabs(x1-x0);
